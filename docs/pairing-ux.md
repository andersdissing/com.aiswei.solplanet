...
A hybrid inverter exposes three subsystems (PV / battery / grid meter); v1.0 ships **four drivers** — one per Energy-tab role plus a derived current-load device for the Homey device list (see [`energy-modeling.md`](./energy-modeling.md)). The user runs **Add device → Solplanet** up to four times — once per driver:

1. **Solplanet Inverter** (Solar tile)
2. **Solplanet Battery** (Battery tile)
3. **Solplanet Grid Meter** (Grid + Home tiles)
4. **Solplanet Home Consumption** (separate device tile for the derived `pv + grid − battery` current load)
...
| `home`  | Same meter-presence check as `meter` — the home current-load formula is `pv + grid − battery`, so a grid meter is required. |
...
| Right IP/serial, but home driver and no grid meter wired | "Inverter reachable but no grid meter is reported. The PV / Home Consumption values are derived from PV + grid − battery, which needs a grid meter." | Skip the Home Consumption pairing. Do not retry. |
...
## Why four add-device steps and not one

Homey routes a pair session's `add_devices` result to the driver that started the session. A single shared "wizard" cannot create devices across drivers in one go. Reusing the validation logic across four drivers (`lib/pairing.js`) is the pragmatic compromise — same form, same client, four quick steps.
