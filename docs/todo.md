...
- [ ] **(Pinned / deferred)** Synthesize a cumulative battery energy counter for the `battery` driver so Homey's Battery + native Home accounting work.
  ...
  - **Refs:** `lib/fields.js` (`parseBatteryData`: `ebi/ebo/eaci/eaco`), `drivers/battery/{device.js,driver.compose.json}`, `HomeyPower.md`, issue [#6](https://github.com/andersdissing/com.aiswei.solplanet/issues/6).
