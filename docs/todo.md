...
- [ ] **Remove the `Solplanet Home Consumption` (`home`) driver entirely.** After the change the Home Consumption value is only visible on the Energy tab's "Home" tile (Homey's cumulative-meter residual) and via `npm run compare`; no per-device tile. Steps: delete `drivers/home/`; drop the `home` role and its label from `lib/pairing.js`; drop the `home` entry from `scripts/sync-views.js` and `create-icon.ps1`; remove the home-image task and refresh the docs (`docs/project.md`, `docs/pairing-ux.md`, `docs/energy-modeling.md`, `README.md`, `readme.txt`) so the app is described as three drivers again. Existing paired Home Consumption devices will need to be removed manually after the deploy.
