...
- [ ] Move the **Home Consumption** value back onto the **Solplanet Grid Meter** device as `measure_power.home` (title "Home Consumption"), and **remove the standalone `home` driver** entirely. Steps: re-add `measure_power.home` to `drivers/meter/driver.compose.json` capabilities + capabilitiesOptions; restore the `_writeHomeConsumption` derivation in `drivers/meter/device.js` (formula `pv + grid_signed − battery_signed`); delete `drivers/home/` and any home-only entries in `lib/pairing.js`, `scripts/sync-views.js`, `create-icon.ps1`; refresh the docs (`docs/project.md`, `docs/pairing-ux.md`, `docs/energy-modeling.md`, `README.md`, `readme.txt`) to show three drivers again. Existing paired Home Consumption devices will need to be removed manually after the deploy; the meter device picks up the new capability via Repair → Refresh.
