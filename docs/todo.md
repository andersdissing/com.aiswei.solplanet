...
- [x] Find a better small / large image for **driver `inverter`** (`drivers/inverter/assets/images/{small,large}.png`, **75×75 / 500×500 PNG, square, white background per Homey guidelines** — pick a product shot of a wall-mounted inverter).
- [x] Find a better small / large image for **driver `battery`** (75×75 / 500×500, square, white background — wall-mount home-battery product shot).
- [x] Find a better small / large image for **driver `meter`** (75×75 / 500×500, square, white background — clean digital-meter front face).
...
- [x] Redraw the driver icons (`drivers/{inverter,battery,meter}/assets/icon.svg`) per Homey guidelines §1.5.2 — line-art with hierarchical stroke widths (outline 4, detail 2, minor detail 1) on a transparent canvas; right-side angle conveyed by the inverter/meter/battery slab construction (outer silhouette + offset inner front face). **Inverter**: angled slab with a 3×3 grid of front-panel cells. **Battery**: angled tall cabinet divided into four stacked modules by horizontal dividers, with a status LED on the top module. **Meter**: angled DIN-rail panel-mount 1-phase energy meter — body with display window, status LED, and four wire-terminal blocks (2 top L/N in + 2 bottom L/N out). Driver tile PNGs (`assets/images/{small,large}.png`) are unchanged — they remain the Pixabay product photos approved in Phase 9.
...
- [~] **Synthesize a cumulative battery energy counter** for the `battery` driver — issue [#10](https://github.com/andersdissing/com.aiswei.solplanet/issues/10) (merged via #11). **Implemented** in `drivers/battery/device.js` (`_accumulateLifetime()`): builds lifetime charged/discharged from the working daily `ebi`/`ebo`, persisted in the device store; prefers the firmware lifetime (`eaci`/`eaco`) when it reports non-zero. Verified on hardware — Homey's `batteryChargedPeriod` now registers (0.1 kWh and rising, vs 0 before). **Remaining:** validate across a full day + a midnight rollover.
