...
const { scanLAN } = require('./discovery');
...
function createPairingHandlers(role) {
  ...
  async function validate(data) {
    ...
    if (role === 'meter' || role === 'home') {
      ...
      if (!hasMeter) {
        const hint = role === 'home'
          ? 'The PV / PV energy total values are derived as PV + grid − battery, which needs a grid meter.'
          : 'Add only the Inverter device.';
        return {
          ok: false,
          error: `Inverter reachable but no grid meter is reported. ${hint}`,
        };
      }
  ...
  return { validate, listDevices, discover };
}
