...
- [x] Expose **Home consumption** as a standalone device — new driver `home` (class `sensor`) with `measure_power` and `meter_power`. Initially derived `pv + grid − battery` from the shared snapshot, then per the next instruction switched to a pure PV display: capabilities now mirror the inverter's PV power and lifetime PV energy. Manifest renamed to "Solplanet PV" (driver id stays `home` to preserve any device already paired); capability titles "PV" and "PV energy total"; pairing label "{model} PV"; meter-presence check dropped since PV-only doesn't need a grid meter.
