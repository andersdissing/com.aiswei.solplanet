...
- [~] Fix `unknown_error_getting_file` when triggering Repair — attempt 1: wrap script in `onHomeyReady(Homey)` (didn't help). Attempt 2: add `template: "repair"` field to the repair entry in driver.compose.json, matching the docs example. Awaiting user re-test.
